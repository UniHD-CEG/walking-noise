...
def get_training_data(data_args, get_waves=False, val_cal_subset=False):
    ...
    # Cache the datasets
    ds_train, ds_test, ds_val = ds_train.cache(), ds_test.cache(), ds_val.cache()
    # Iterate through all datasets once to make sure the caching completes
    _ = list(ds_train.as_numpy_iterator())
    _ = list(ds_test.as_numpy_iterator())
    _ = list(ds_val.as_numpy_iterator())
    del _
    # Make sure these unused lists get removed from memory before we move on
    gc.collect()
